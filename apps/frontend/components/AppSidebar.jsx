...
const companyItems = [
  ...
  { href: "/company/CreateProposalSection", label: "Create Project Proposal" },
  { href: "/company/ProposalDetailsSection", label: "Proposal Details" },
  { href: "/company/KanbanBoardPage", label: "Kanban Board" },
  { href: "/company/PrdRepository", label: "PRD Repository" },
  { href: "/company/Prd-details&Editor", label: "PRD Details and Editors" },
...
export default function AppSidebar({ section = "client" }) {
  ...
  return (
    <aside className="h-full min-h-0 bg-[#111827] text-white flex flex-col shrink-0 border-r border-white/5 w-[280px]">
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto flex flex-col py-4 space-y-3 px-4">
        {menuItems.map((item) => {
